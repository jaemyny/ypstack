...
설치 후 Claude Code 채팅창에 자연어로 질문하면 됩니다. Claude가 알맞은 MCP를 골라(필요하면 여러 개를 교차) 호출합니다.

| 질문 예시 | 사용 MCP |
|-----------|----------|
| "잠실엘스 33평 KB 시세와 최근 2년 추이 보여줘" | `kb-price-mcp` |
| "강남구 2024년 아파트 매매 실거래 TOP 10" | `stats-realty-mcp` |
| "현재 기준금리랑 주택담보대출 금리 추이 알려줘" | `stats-finance-mcp` |
| "분당구 영어학원 수·학생 수와 인근 미세먼지" | `stats-edu-mcp` + `stats-env-mcp` |
| "마포구 인구 + 학원 수 + 미세먼지 + 상권 임대료 한 번에 정리해줘" | Claude가 4개 MCP 자동 교차 호출 |
