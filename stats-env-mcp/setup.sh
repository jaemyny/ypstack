...
echo "======================================"
echo " stats-env-mcp 설치 시작"
echo "======================================"
echo ""
echo " 대기질·공원·환경통계 (에어코리아, 서울시, KOSIS)"
echo ""
...
echo ""
echo "  ┌─ DATA_GO_KR (공공데이터포털) — 에어코리아 ─────────"
echo "  │  발급: https://www.data.go.kr → '에어코리아 대기오염' 검색"
...
if [ -f "$MCP_JSON" ]; then
    if grep -q '"stats-env"' "$MCP_JSON" 2>/dev/null; then
        echo "      이미 stats-env 설정이 존재합니다."
