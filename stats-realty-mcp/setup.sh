...
echo "======================================"
echo " stats-realty-mcp 설치 시작"
echo "======================================"
echo ""
echo " 아파트 실거래·전월세·단지·주택인허가·가격지수"
echo ""
...
echo "  │  발급: https://www.data.go.kr → '아파트매매실거래자료' 검색"
...
if [ -f "$MCP_JSON" ]; then
    if grep -q '"stats-realty"' "$MCP_JSON" 2>/dev/null; then
        echo "      이미 stats-realty 설정이 존재합니다."
