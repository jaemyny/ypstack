...
## 부록: 추가 엔드포인트 발굴 (C1 후보, 2026-06)

kbland.kr 의 JS 번들에서 `land-*` 엔드포인트 189개를 추출(`https://kbland.kr/` → `<script src>` JS → 정규식 `land-[A-Za-z]+/[A-Za-z]+/[A-Za-z0-9]+`)해, PRD §5.2.1 의 4개 확장 후보를 조사한 결과:

| 기능 | 엔드포인트 | 상태 | 비고 |
|---|---|---|---|
| **매물수(현재)** | `land-complex/complexResteBrhs/propCountByTradeKind` | ✅ **확인** | param `단지기본일련번호` → `{매매건수, 전세건수, 월세건수, 전체건수}`. (단, **시계열 "추이"는 없음**. 현재 건수는 이미 `complex/main`·`kb_get_complex_price` 에도 노출됨) |
| **학군(주변 학교)** | `land-complex/honeyLocation/schoolMarkerList` | ⚠️ **파라미터 미상** | 경로·응답 게터(`학교목록` 등)는 확인. 요청 파라미터를 못 찾음(추측 전부 `10400 BAD_REQUEST`). **HAR 필요** |
| **편의시설** | `land-complex/honeyLocation/{academyMarkerList, hospitalMarkerList, subwayMarkerList, starbucksMarkerList}` | ⚠️ **파라미터 미상** | 응답 게터 확인(`학원개수/학원목록`, `병원개수/병원목록`, `지점명` 등). 요청 파라미터 동일하게 미상. **HAR 필요** |
| **학교 상세** | `land-complex/complexSchool/dtail` | ◑ 부분 | param `{식별자(=마커 id), 학교과정분류구분, 단지기본일련번호}`. 마커 선택 후 상세용 → markerList 선행 필요 |
| **보유세 시뮬** | (없음) | ❌ **API 없음** | 직접 엔드포인트 없음. 공시지가 `land-complex/complex/pubLandPriceByDong` 만 존재. 보유세는 공시가격 기반 클라이언트 계산이 필요(별도 과제) |

**남은 블로커 — honeyLocation 요청 파라미터:** minified JS의 호출부가 메인 번들에 없어 추출 실패. kbland.kr 단지 상세 → "꿀위치(주변)" 탭에서 학교/학원 마커가 로드될 때의 **실제 요청 쿼리스트링을 HAR로 1건만 캡처**하면 즉시 구현 가능 (캡처 절차는 위 "HAR 재캡처 워크플로" 참고, 대상 요청 = `honeyLocation/schoolMarkerList`).
